...
def check_crl(crl_url, serial_number):
    """
    Vérifie si un certificat est révoqué en consultant la CRL (Certificate Revocation List).
    Tente d'abord de charger la CRL en DER, et en cas d'échec, en PEM.
    :param crl_url: URL de la CRL.
    :param serial_number: Numéro de série du certificat à vérifier.
    :return: True si révoqué, False si non révoqué, None si la vérification échoue.
    """
    try:
        # Télécharger la CRL depuis l'URL
        response = requests.get(crl_url, timeout=5)
        if response.status_code == 200:
            try:
                # Essayer de charger la CRL en DER
                crl = x509.load_der_x509_crl(response.content)
            except Exception:
                # Si l'encodage DER échoue, tenter en PEM
                crl = x509.load_pem_x509_crl(response.content)
            # Parcourir tous les certificats révoqués dans la CRL
            for revoked_cert in crl:
                if revoked_cert.serial_number == serial_number:
                    return True  # Le certificat est révoqué
        return False  # Le certificat n'est pas trouvé dans la CRL
    except Exception:
        return None  # La vérification n'a pas pu être effectuée
