...
async function editTemplate(template) {
  try {
    ...
    try {
      ...
      console.log("✅ [DEBUG] PDF original téléchargé avec succès");
    } catch (pdfError) {
      console.error("❌ [DEBUG] Erreur lors du téléchargement du PDF original:", pdfError);
    }
    ...
    if (originalPdfBlob) {
      ...
      console.log("✅ [DEBUG] File PDF créé:", originalFile);
    }
    ...
    if (templateDetails.signature_image) {
      try {
        signatureImageUrl = templateDetails.signature_image;
        console.log("✅ [DEBUG] URL de l'image de signature:", signatureImageUrl);
      } catch (signatureError) {
        console.error("❌ [DEBUG] Erreur lors de la récupération de l'image de signature:", signatureError);
      }
    }
    ...
    console.log("✅ [DEBUG] Template préparé pour édition:", editingTemplate.value);
    
    // Ouvrir la modale d'édition
    showEditModal.value = true;
  } catch (error) {
    console.error("❌ [DEBUG] Erreur lors de la récupération des détails du template:", error);
...
async function viewPendingDocument(doc) {
  ...
  try {
    ...
    // Utiliser les endpoints API sécurisés pour le téléchargement
    if (response.data && (response.data.generated_pdf_url || response.data.document_file_url)) {
      console.log('Document trouvé, téléchargement via endpoint API sécurisé');
      
      // Utiliser l'endpoint de téléchargement sécurisé
      let downloadEndpoint = '';
      let filename = '';
      
      if (response.data.generated_pdf_url) {
        // PDF généré disponible
        downloadEndpoint = `https://ppd.camgovca.cm/api/documents/qr-positions/${doc.id}/download_generated_pdf/`;
        filename = `${doc.document_name || doc.name || 'document'}_generated.pdf`;
        console.log('Téléchargement du PDF généré via endpoint sécurisé');
    } else {
        // Document original seulement
        downloadEndpoint = `https://ppd.camgovca.cm/api/documents/qr-positions/${doc.id}/download_document/`;
        filename = `${doc.document_name || doc.name || 'document'}_original.pdf`;
        console.log('Téléchargement du document original via endpoint sécurisé');
      }
      
      // Télécharger via l'endpoint API sécurisé
  ...
  try {
    ...
    if (error.response) {
      switch (error.response.status) {
        case 404:
          errorMessage = 'Document signé introuvable. Le document n\'a peut-être pas encore été signé.';
          break;
        case 403:
          errorMessage = 'Accès refusé. Vous n\'avez pas les permissions pour télécharger ce document.';
          break;
        case 500:
          errorMessage = 'Erreur serveur. Veuillez réessayer plus tard.';
          break;
        default:
          errorMessage = `Erreur ${error.response.status}: ${error.response.data?.detail || 'Erreur inconnue'}`;
      }
    }
